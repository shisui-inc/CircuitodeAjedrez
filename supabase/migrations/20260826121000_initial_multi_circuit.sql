...
on conflict (id) do update set status = 'finalizado', is_published = true;
...
create table if not exists public.tournaments (
  id text primary key default gen_random_uuid()::text,
  circuit_id text not null default 'circuito-paranaense-2026' references public.circuits(id) on delete cascade,
  name text not null,
...
create unique index if not exists tournaments_circuit_round_key
  on public.tournaments(circuit_id, round);
